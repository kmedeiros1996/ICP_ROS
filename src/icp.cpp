...
Eigen::MatrixXd ICP::BestFitTransform(const Eigen::MatrixXd &pointcloud_A, const Eigen::MatrixXd &pointcloud_B) {
...
double ICP::EuclideanDistance(const Eigen::Vector3d &pt_a, const Eigen::Vector3d &pt_b) {
  return sqrt(pow(pt_a[0]-pt_b[0], 2) + pow(pt_a[1]-pt_b[1], 2) + pow(pt_a[2]-pt_b[2], 2) );
}

void ICP::CalcClosestNeighborsKDTree(const Eigen::MatrixXd &src, const nn_kd_tree &dst_tree)
{
...
void ICP::Run(const Eigen::MatrixXd &pointcloud_A, const Eigen::MatrixXd &pointcloud_B, double tolerance)
{
  ...
  for (iters_ = 1; iters_ <= max_iters_; iters_++) {
    ...
    CalcClosestNeighborsKDTree(src_3d_.transpose(), dst_index);

    for (int j = 0; j < a_rows; j++) {
        closest_pts_in_dst.block<3,1>(0,j) = hg_dst.block<3,1>(0,indices_[j]);
    }

    transform_matr_ = BestFitTransform(src_3d_.transpose(), closest_pts_in_dst.transpose());
  ...
  transform_matr_ = BestFitTransform(pointcloud_A, src_3d_.transpose());
}
